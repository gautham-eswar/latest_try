...
def _generate_education_section(education_list: Optional[List[Dict[str, Any]]]) -> Optional[str]:
    ...
    for edu in education_list:
        ...
        if uni and degree_str:
            ...
            item_list_started = False
            if additional_info_raw:
                content_lines.append(r"      \resumeItemListStart")
                item_list_started = True
                content_lines.append(f"        \\resumeItem{{{fix_latex_special_chars(additional_info_raw)}}}")
            elif relevant_coursework_raw and isinstance(relevant_coursework_raw, list):
                content_lines.append(r"      \resumeItemListStart")
                item_list_started = True
                ...
                content_lines.append(r"      \resumeItemListStart")
                item_list_started = True
                content_lines.append(f"        \\resumeItem{{Relevant Coursework: {fix_latex_special_chars(relevant_coursework_raw)}}}")

            known_keys = {"institution", "university", "location", "degree", "specialization", "start_date", "end_date", "dates", "gpa", "honors", "additional_info", "relevant_coursework"}
            for k, v in edu.items():
                if k in known_keys:
                    continue
                if isinstance(v, str) and v.strip():
                    if not item_list_started:
                        content_lines.append(r"      \resumeItemListStart")
                        item_list_started = True
                    content_lines.append(f"        \\resumeItem{{{fix_latex_special_chars(v)}}}")

            if item_list_started:
...
def _generate_experience_section(experience_list: Optional[List[Dict[str, Any]]], tech_skills: List[str], metrics: List[str]) -> Optional[str]:
    ...
    for exp in experience_list:
        ...
        loc_str = _parse_location_dict(exp.get("location"))

        date_val = exp.get("dates") or exp.get("date")
        dates_str = ""
        ...
        if company and position:
            content_lines.append(f"    \\resumeSubheading{{{company}}}{{{dates_str}}}{{{position}}}{{{loc_str}}}")
            
            responsibilities = exp.get("responsibilities/achievements") or exp.get("responsibilities") or exp.get("achievements") or []
            if responsibilities:
...
def _generate_awards_section(awards_list: Optional[List[Dict[str, Any]]]) -> Optional[str]:
    ...
    for award in awards_list:
        ...
        content_lines.append(f"    \\resumeSubheading{{{title}}}{{{date}}}{{{issuer}}}{{}}")
    if not content_lines: return None
