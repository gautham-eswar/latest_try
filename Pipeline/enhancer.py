...
class ResumeEnhancer:
    ...
    def _enhance_bullet_with_keywords(self, bullet: str, keywords: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            # Clean up the response (remove label prefixes and quotes if present)
            enhanced_bullet = re.sub(
                r'^(?:\s*(?:enhanced\s*bullet(?:\s*point)?|final\s*bullet|rewritten\s*bullet|updated\s*bullet|output)\s*:\s*)+',
                '',
                enhanced_bullet,
                flags=re.IGNORECASE
            ).strip()
