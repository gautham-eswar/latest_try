...
def download_resume(app, resume_id, format_type):
    ...
    if isinstance(db, FallbackDatabase):
        ...
        final_resume_latex_from_db = None  # Initialize LaTeX content variable
        try:
            ...
            response_enhanced = (
                db.table("resumes")
                .select("data, id, created_at, final_resume_latex")  # Updated to use final_resume_latex
            ...
            if response_enhanced.data:
                ...
                final_resume_latex_from_db = response_enhanced.data[0].get("final_resume_latex")  # Get stored LaTeX content
                ...
                response_direct = (
                    db.table("resumes")
                    .select("data, id, final_resume_latex")  # Updated to use final_resume_latex
                    .eq("id", resume_id)
                    .limit(1)
                    .execute()
                )
                if response_direct.data:
                    ...
                    final_resume_latex_from_db = response_direct.data[0].get("final_resume_latex")  # Get stored LaTeX content
                    ...
                    final_resume_latex_from_db = None
            ...
            # resume_data_to_use remains None
            final_resume_latex_from_db = None  # Ensure LaTeX content is None on error
    ...
    if format_type == "json":
        ...
        try:
            logger.info(f"Serving stored LaTeX for resume ID: {resume_id}")
            
            # Use stored LaTeX content - should always be available since PDF generation stores it
            if final_resume_latex_from_db:
                latex_content = final_resume_latex_from_db
                logger.info(f"Successfully retrieved stored LaTeX content for resume ID: {resume_id}")
            else:
                logger.error(f"No stored LaTeX content found for resume ID: {resume_id} - this should not happen with automatic PDF generation")
                return app.create_error_response(
                    "LatexNotFound", f"LaTeX content not found for resume ID {resume_id}", 404
                )
