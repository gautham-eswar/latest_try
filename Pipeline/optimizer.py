...
def enhance_resume(job_id, resume_id, user_id, job_description_text, generate_summary: bool = False):
    ...
    match_results = matcher.process_keywords_and_resume(
        ...
        overall_skill_limit=30     # Lower cap to keep Skills concise and scannable
    )
    ...
    try:
        ...
        try:
            ...
            system_prompt = (
                ...
                "(1) For each category, assign a score 0–10, multiply by the fixed weight, compute the weighted contribution, and give a 1–2 sentence justification. Calibrate scores to realistic hiring bands. Reward transferable evidence even if exact keywords are missing. Avoid extremely low totals (e.g., <25) unless the resume is clearly unrelated to the JD. "
                ...
                "Calibration bands to guide final totals: 90–100 exceptional; 75–89 strong; 60–74 solid; 45–59 developing; 30–44 limited; <30 poor. "
                "(3) Compute the total weighted score out of 100. "
                "When the JD emphasizes functions (e.g., research, client engagement, analysis) more than tools, map those functions under Skills Match rather than penalizing for missing tools. For each category explanation, cite one concrete resume element and one JD element. "
